...
class HistorySuccessCompanyActivity : AppCompatActivity() {
    val Context.dataStore: DataStore<Preferences> by preferencesDataStore(name = "user")
    private var token = ""
    private lateinit var userPreferences: UserPreferences
    private lateinit var binding: ActivityHistorySuccessCompanyBinding
    private lateinit var adapter: CompanyAccHistoryAdapter
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityHistorySuccessCompanyBinding.inflate(layoutInflater)
                ...
                client.enqueue(object : Callback<HistoryResponse> {
                    override fun onResponse(
                        ...
                        if (response.isSuccessful) {
                            ...
                            binding.userSuccessList.layoutManager =
                                LinearLayoutManager(this@HistorySuccessCompanyActivity)
                            binding.userSuccessList.adapter = adapter
                        } else {
